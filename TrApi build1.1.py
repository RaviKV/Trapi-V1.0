   ...
   || ||   //      \\ ||        ||
    ...
    def download_common_wordlist(self):
        ...
        try:
            ...
            print(f"Common word list downloaded to: {os.path.abspath(wordlist_path)}")
    ...
    def get_user_input(self):
        ...
        while True:
            self.output_path = input(f"Enter the path along with file name including the extension (xml/json) to save the output ({self.output_format}): ")
            if self.validate_output_path(self.output_path, self.output_format):
                break
            else:
                print(f"Invalid path or filename. Please ensure the path is correct and the filename ends with .{self.output_format}.")

    def validate_api_url(self, url):
        return re.match(r'^https?://[^\s/$.?#].[^\s]*$', url)

    def validate_output_path(self, path, file_format):
        # Validate if the directory exists and the filename has the correct extension
        directory = os.path.dirname(path)
        filename = os.path.basename(path)
        return (directory == '' or os.path.exists(directory)) and filename.endswith(f'.{file_format}')
    ...
    def xss_check(self, endpoint):
        ...
        try:
            response = requests.get(f"{endpoint}?q={xss_payload}")
            if xss_payload in response.text:
                return "Potential Cross-Site Scripting (XSS) vulnerability"
        except:
            pass
        return "Not vulnerable"

    def insecure_deserialization_check(self, endpoint):
        serialized_payload = {'data': 'O:8:"stdClass":0:{}'}
        try:
